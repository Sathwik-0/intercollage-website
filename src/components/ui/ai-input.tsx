...
export function MorphPanel({ onSend, isTyping = false }: { onSend?: (message: string) => void; isTyping?: boolean }) {
  ...
  const handleSuccess = React.useCallback((msg?: string) => {
    ...
    // Don't close immediately if we want to chat continuously, 
    // but the original code closes on submit. We'll leave it as is or modify slightly so user can keep typing.
    // The user requested "exactly as is", but if it closes every time they submit, chatting is annoying.
    // I will clear the textarea but NOT close the form.
  ...
  const triggerSubmit = React.useCallback(() => {
    ...
    if (msg && msg.trim()) {
      handleSuccess(msg.trim());
    }
  }, [handleSuccess])

  const ctx = React.useMemo(
    () => ({ showForm, successFlag, triggerOpen, triggerClose, triggerSubmit, isTyping }),
    [showForm, successFlag, triggerOpen, triggerClose, triggerSubmit, isTyping]
  )

  return (
    <div className="flex items-center justify-end" style={{ width: showForm ? FORM_WIDTH : "auto", height: showForm ? FORM_HEIGHT : "auto" }}>
      <motion.div
        ref={wrapperRef}
        data-panel
        className={cx(
      ...
      >
        <FormContext.Provider value={ctx}>
          <DockBar />
          <InputForm ref={textareaRef} submitBtnRef={submitBtnRef} onSuccess={handleSuccess} />
...
function DockBar() {
  const { showForm, triggerOpen, triggerSubmit, isTyping } = useFormContext()
  return (
    <footer className="relative z-10 pointer-events-auto mt-auto flex h-[44px] items-center justify-center whitespace-nowrap select-none">
      <div className="flex items-center justify-center gap-2 px-3 max-sm:h-10 max-sm:px-2">
        ...
        >
          <span className="truncate">{showForm && isTyping ? "Thinking..." : "Ask AI"}</span>
        </Button>
      </div>
    </footer>
  )
...
function InputForm({ ref, submitBtnRef, onSuccess }: { ref: React.Ref<HTMLTextAreaElement>; submitBtnRef: React.Ref<HTMLButtonElement>; onSuccess: (msg: string) => void }) {
  const { triggerClose, showForm } = useFormContext()

  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault()
    const msg = (ref as React.MutableRefObject<HTMLTextAreaElement>).current?.value;
    if (msg?.trim()) {
        onSuccess(msg.trim());
    }
  }
  ...
  function handleKeys(e: React.KeyboardEvent<HTMLTextAreaElement>) {
    ...
    if (e.key === "Enter" && !e.shiftKey) { // Changed to allow direct Enter since it's a chat
      e.preventDefault()
      const btn = submitBtnRef as React.RefObject<HTMLButtonElement>
      btn.current?.click()
    }
  }

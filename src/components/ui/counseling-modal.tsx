...
export function CounselingModal({ variant, children }: CounselingModalProps) {
  ...
  return (
    <Dialog>
      ...
      <DialogContent 
        className="sm:max-w-[550px] w-[95vw] p-0 border border-emerald-500/20 shadow-2xl overflow-hidden bg-white/70 backdrop-blur-3xl rounded-3xl !zoom-in-100 data-open:slide-in-from-bottom-3 duration-300"
      >
        {isSubmitted ? (
          ...
          <div className="flex flex-col max-h-[85vh] overflow-y-auto">
            {/* Header Block */}
            <div className="bg-gradient-to-br from-[#044e3b] to-[#04241a] p-8 md:p-10 relative overflow-hidden shrink-0">
              ...
              <DialogDescription className="text-emerald-100/80 mt-2 font-medium text-sm md:text-base max-w-sm relative z-10 leading-relaxed">
                {config.description}
              </DialogDescription>
            </div>
            ...
            <div className="p-6 md:p-8 bg-white/50">
              <form onSubmit={(e) => { e.preventDefault(); setIsSubmitted(true); }} className="space-y-5">
                  ...
                  <div key={field.name} className="flex flex-col gap-1.5">
                    ...
                    {field.type === "textarea" ? (
                      ...
                      <Input 
                        ...
                        type={field.type} 
                        placeholder={field.placeholder}
                        className="bg-white/80 border-emerald-100 focus-visible:ring-emerald-500/20"
                      />
                    )}
                  </div>
                ))}
                ...
                <Button 
                  type="submit" 
                  className="mt-4 w-full h-11 bg-[#044e3b] hover:bg-[#033b2c] text-white font-bold rounded-xl shadow-[0_8px_16px_rgba(4,78,59,0.2)] transition-all hover:-translate-y-px hover:shadow-[0_12px_20px_rgba(4,78,59,0.3)]"
                >
                  {config.ctaText}
                </Button>
              </form>

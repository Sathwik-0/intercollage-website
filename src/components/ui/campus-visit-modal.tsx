...
export function CampusVisitModal({ children }: { children: React.ReactNode }) {
  ...
  return (
    <Dialog>
      ...
      */}
      <DialogContent 
        className="sm:max-w-[700px] w-[95vw] p-0 border border-emerald-500/20 shadow-2xl overflow-hidden bg-white/60 backdrop-blur-2xl rounded-3xl data-open:zoom-in-100 data-open:slide-in-from-bottom-3 duration-300"
      >
        ...
        {isSubmitted ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-[1fr_1.5fr] h-full max-h-[85vh] overflow-y-auto">
            ...
            <div className="bg-gradient-to-br from-[#044e3b] to-[#04241a] p-8 flex flex-col justify-between relative overflow-hidden">
              <div className="absolute top-0 right-0 w-64 h-64 bg-emerald-500/10 rounded-full blur-3xl -translate-y-1/2 translate-x-1/3" />
              <div className="absolute bottom-0 left-0 w-48 h-48 bg-emerald-400/10 rounded-full blur-2xl translate-y-1/3 -translate-x-1/4" />
              
              <div className="relative z-10">
                <span className="inline-flex items-center gap-1.5 rounded-full border border-emerald-500/30 bg-emerald-500/10 px-3 py-1 text-[10px] font-bold text-emerald-100 uppercase tracking-wider mb-6">
                ...
                <p className="text-emerald-100/80 font-medium text-sm mt-3 leading-relaxed">
              ...
              <div className="relative z-10 mt-8 space-y-4">
                <div className="flex items-center gap-3 text-emerald-50">
                  ...
                  <span className="text-sm font-semibold text-emerald-100">1-on-1 Counselor Meeting</span>
                </div>
                <div className="flex items-center gap-3 text-emerald-50">
                  <div className="size-8 rounded-full bg-emerald-500/20 flex items-center justify-center shrink-0">
                    <MapPin className="size-4" />
                  </div>
                  <span className="text-sm font-semibold text-emerald-100">Guided Infrastructure Tour</span>
                </div>
                <div className="flex items-center gap-3 text-emerald-50">
                  <div className="size-8 rounded-full bg-emerald-500/20 flex items-center justify-center shrink-0">
                    <Calendar className="size-4" />
                  </div>
                  <span className="text-sm font-semibold text-emerald-100">Tailored Academic Blueprint</span>
                </div>
              </div>
            </div>
              ...
              <form onSubmit={handleSubmit} className="flex flex-col gap-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="flex flex-col gap-1.5">
                    ...
                    <Input required placeholder="e.g., 10th standard" className="bg-white/80 border-emerald-100 focus-visible:ring-emerald-500/20" />
                  </div>
                  <div className="flex flex-col gap-1.5">
                    <Label className="text-xs font-bold text-[#406b5c]">Preferred Stream</Label>
                    <Input placeholder="MPC, BiPC, etc." className="bg-white/80 border-emerald-100 focus-visible:ring-emerald-500/20" />
                  </div>
                </div>

                <div className="flex flex-col gap-1.5">
                  <Label className="text-xs font-bold text-[#406b5c]">Preferred Visit Date</Label>
                  <Input type="date" className="bg-white/80 border-emerald-100 focus-visible:ring-emerald-500/20 text-[#04241a]" />
                </div>
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <textarea 
                    ...
                    className="flex min-h-[80px] w-full rounded-lg border border-emerald-100 bg-white/80 px-3 py-2 text-sm text-[#04241a] shadow-sm shadow-black/5 transition-shadow placeholder:text-slate-400 focus-visible:outline-none focus-visible:ring-[3px] focus-visible:ring-emerald-500/20 resize-none" 
                  />
                </div>

                <Button 
                  type="submit" 
                  className="mt-2 w-full h-11 bg-[#044e3b] hover:bg-[#033b2c] text-white font-bold rounded-xl shadow-[0_8px_16px_rgba(4,78,59,0.2)] transition-all hover:translate-y-[-1px] hover:shadow-[0_12px_20px_rgba(4,78,59,0.3)]"
                >
